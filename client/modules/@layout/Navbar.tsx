...
const NavBar = () => {
  ...
  const { data: brandsData = [], isSuccess: brandsSuccess } =
    useGetBrandsQuery();
  const { data: bikeTypesData = [], isSuccess: typesSuccess } =
    useGetBikeTypesQuery();

  const brands = brandsSuccess
  ...
  const bikeTypes = typesSuccess
    ? bikeTypesData.map((type) => ({
        title: type.name,
  ...
  const navData = [
    { title: "Bike Reviews", link: "/bike-reviews" },
    { title: "Types", link: "/type", children: bikeTypes },
    { title: "Brands", link: "/brands", children: brands },
  ];
